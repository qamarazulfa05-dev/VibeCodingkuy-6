document.addEventListener("DOMContentLoaded", function () {
    ...
    const libraryServiceStatus = document.getElementById("libraryServiceStatus");
    ...
    // library-service, bukan dari localStorage.)
    // ==================================================
    ...
    async function panggilAPI(url, options = {}) {
        ...
        try {
            ...
            throw new Error("Tidak dapat menghubungi layanan. Periksa apakah service sudah berjalan.");
        }
    ...
    // ==================================================
    // CEK STATUS SERVICE (opsional, untuk debugging /
    // menunjukkan bahwa ini benar dua service terpisah)
    // ==================================================

    async function cekStatusService() {
        try {
            const res = await fetch(CONFIG.AUTH_SERVICE_URL + "/api/health");
            if (res.ok) {
                authServiceStatus.textContent = "online";
                authServiceStatus.className = "status-ok";
            } else {
                throw new Error();
            }
        } catch (error) {
            authServiceStatus.textContent = "offline";
            authServiceStatus.className = "status-down";
        }

        try {
            const res = await fetch(CONFIG.LIBRARY_SERVICE_URL + "/api/health");
            if (res.ok) {
                libraryServiceStatus.textContent = "online";
                libraryServiceStatus.className = "status-ok";
            } else {
                throw new Error();
            }
        } catch (error) {
            libraryServiceStatus.textContent = "offline";
            libraryServiceStatus.className = "status-down";
        }
    }


    // ==================================================
    // LOGIN (memanggil AUTH-SERVICE)
    // ==================================================
    ...
    loginButton.addEventListener("click", async function () {
        ...
        // dilakukan di auth-service, ini hanya agar pengguna
        ...
        try {
            ...
            const response = await fetch(CONFIG.AUTH_SERVICE_URL + "/api/auth/login", {
                method: "POST",
            ...
            loginError.textContent = error.message || "Tidak dapat menghubungi auth-service.";
        } finally {
    ...
    // ==================================================
    // RESTART SISTEM (memanggil LIBRARY-SERVICE)
    // ==================================================
    ...
    restartButton.addEventListener("click", async function () {
        ...
        try {
            await panggilAPI(CONFIG.LIBRARY_SERVICE_URL + "/api/system/reset", {
                method: "POST"
            });
    ...
    // ==================================================
    // MUAT DAFTAR BUKU (GET dari LIBRARY-SERVICE)
    // ==================================================
    ...
    async function muatDaftarBuku() {
        ...
        try {
            daftarBuku = await panggilAPI(CONFIG.LIBRARY_SERVICE_URL + "/api/books");
        } catch (error) {
            bookList.innerHTML = "";
    ...
    // ==================================================
    // PROSES PEMINJAMAN (POST ke LIBRARY-SERVICE)
    // library-service akan memverifikasi token ini dengan
    // memanggil auth-service lewat API sebelum memproses.
    // ==================================================
    ...
    async function pinjamBuku(idBuku, tombol) {
        ...
        try {
            ...
            await panggilAPI(CONFIG.LIBRARY_SERVICE_URL + "/api/loans", {
                method: "POST",
    ...
    // ==================================================
    // INFORMASI PEMINJAMAN (GET dari LIBRARY-SERVICE)
    // ==================================================
    ...
    async function muatInformasiPeminjaman() {
        ...
        try {
            data = await panggilAPI(CONFIG.LIBRARY_SERVICE_URL + "/api/loans");
        } catch (error) {
